...
/**
 * Provides the communication threads and message delivering/handling for a
 * single connected Bluetooth Classic device. More information on the Serial
 * Port Profile (SPP) can be found at
...
public class RNBluetoothClassicService {
    ...
    /**
     * Communication thread takes over once ConnectThread has successfully handed
     * off.
     */
    ...
    void write(byte[] out) {
        if (D)
            Log.d(TAG, "Write in service, state is " + DeviceState.CONNECTED.name());
    ...
    synchronized void stop() {
        if (D)
            Log.d(TAG, "Stopping RNBluetoothClassic service");
    ...
    private synchronized void connectionSuccess(BluetoothSocket socket, BluetoothDevice device) {
        if (D)
            Log.d(TAG, String.format("Connected to %s", device.getAddress()));
     ...
     * <li>Initializes the notifies that the connection was successful allowing the
     * service to continue.</li>
     * </ul>
     */
    private class ConnectThread extends Thread {
         ...
         *
         * TODO update to isConnecting/isRunning to remove the forced socket
         * close/exception
         */
        void cancel() {
            try {
    ...
    /**
     * ConnectedThread runs throughout a device connection/registration. It's
     * responsible for sending and receiving data from the device and passing it
     * along to the appropriate handlers.
     * <p>
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            // Keep listening to the InputStream while connected, the thread is currently
            // cancelled
